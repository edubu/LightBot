...
def get_arm_angles(landmarks_front, landmarks_right, landmarks_hand):
    """
    ...
    elbow = joints[1]
    wrist = joints[2]
    hand = [landmarks_hand[9].x, landmarks_hand[9].y]

    # shoulder angle in xy plane
    shoulder_xy = calculate_angle([shoulder[0], shoulder[1] + 0.1], shoulder[:2], elbow[:2])
    # shoulder angle in yz plane
    shoulder_yz = calculate_angle([shoulder[2], shoulder[3] + 0.1], shoulder[2:4], elbow[2:4])
    elbow_angle = calculate_angle(shoulder[0:3], elbow[0:3], wrist[0:3])
    wrist_angle = calculate_angle(elbow[2:4], wrist[2:4], hand)
    ...
    while True:
        ...
        if results1.pose_landmarks:
            for i, landmark in enumerate(results1.pose_landmarks.landmark):
                ...
                pxl_x = int(round(pxl_x))
                pxl_y = int(round(pxl_y))
                cv2.circle(frame1, (pxl_x, pxl_y), 3, (0, 0, 255), -1)  # add keypoint detection points into figure
        ...
        if results0.pose_landmarks and results1.pose_landmarks and results1.right_hand_landmarks:
            shoulder_xy, shoulder_yz, elbow_angle, wrist_angle = get_arm_angles(results0.pose_landmarks.landmark,
                                                                                ...
                                                                                results1.right_hand_landmarks.landmark)
